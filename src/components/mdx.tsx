...
function TechItem({ icon, label, color }: TechItemProps) {
  return (
    <Row gap="8" vertical="center" title={label}>
      <Icon
        name={icon}
        style={{
          color: color || "var(--neutral-on-background-strong)",
        }}
      />
...
function TechStack({ children }: { children: ReactNode }) {
  return (
    <Row wrap gap="12" paddingTop="8" paddingBottom="16" vertical="center">
      {children}
    </Row>
  );
}
